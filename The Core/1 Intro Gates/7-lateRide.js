...
on your route home are up, leaving you stranded! Unfortunately,
...
The duration of your ride, in minutes.
It is guaranteed that you've been riding for less than a day (24 hours).
...
function lateRide(n) {
  ...
  const sumHours = hours.toString().split('').map(Number).reduce((prev, curr) => prev + curr, 0);
  const sumMinutes = minutes.toString().split('').map(Number).reduce((prev, curr) => prev + curr, 0);
