...
The second time you pay for the card, your pass is extended by 28 days, i.e.
the number of days in February (note that leap years are not considered),
...
Figure out the number of days by which your pass will now be extended, and return
...
This number can be equal to 28, 30 or 31.
...
function metroCard(lastNumberOfDays) {
  ...
  for (let i = 0; i < monthDays.length; i += 1) {
    if (monthDays[i] === lastNumberOfDays) {
      if (monthDays[i + 1] !== undefined) {
        answer.push(monthDays[i + 1]);
      }
    }
  }
